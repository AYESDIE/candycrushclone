	...
	for i in width:
		for j in height:
			if !restriced_fill(Vector2(i, j)):
				...
				# Place that piece in the position
...
func collapse_columns():
	for i in width:
		for j in height:
			if all_pieces[i][j] == null && !restriced_fill(Vector2(i, j)):
